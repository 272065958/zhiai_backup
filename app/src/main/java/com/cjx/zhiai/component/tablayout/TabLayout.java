...
public class TabLayout extends HorizontalScrollView {
    ...
    private final int[] APPCOMPAT_CHECK_ATTRS = { android.support.design.R.attr.colorPrimary };
...
//        void onTabReselected(Tab tab);
    ...
    public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mIndicatorBottom = res.getDimensionPixelOffset(R.dimen.block_line_height);
    }

    int mIndicatorBottom;
    // 设置滚动横条的宽度和离底部的高度
    public TabLayout setIndicatorWidthAndBottom(int width, int bottom){
...
//            setPadding(mTabPaddingStart, mTabPaddingTop, mTabPaddingEnd, mTabPaddingBottom);
//            ViewCompat.setPaddingRelative(this, mTabPaddingStart, mTabPaddingTop,
//                    mTabPaddingEnd, mTabPaddingBottom);
//            setGravity(Gravity.CENTER);
//            setOrientation(VERTICAL);
            setClickable(true);
            setBackgroundResource(R.drawable.item_transparent_pressed_bg);
        }
...
//                            .inflate(android.support.design.R.layout.design_layout_tab_icon, this, false);
                    ...
                    int size = (int) (1.5*mIndicatorBottom);
                    LayoutParams rlp = new LayoutParams(size, size);
                    rlp.addRule(RelativeLayout.RIGHT_OF, R.id.tag_view);
                    rlp.addRule(RelativeLayout.ALIGN_TOP, R.id.tag_view);
                    rlp.leftMargin = mIndicatorBottom / 2;
                    iconView.setLayoutParams(rlp);
                    addView(iconView);
                    mIconView = iconView;
                }
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        private void setIndicatorPosition(int left, int right) {
            if (left != mIndicatorLeft || right != mIndicatorRight) {
                ...
                // cjx 改动 固定Indicator的宽度
                if(mIndicatorBottom > 0){
                    mIndicatorLeft = mIndicatorLeft + mIndicatorBottom;
                    mIndicatorRight = mIndicatorRight - mIndicatorBottom;
                }
                ViewCompat.postInvalidateOnAnimation(this);
            }
        }
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                canvas.drawRect(mIndicatorLeft, getHeight() - mSelectedIndicatorHeight - mIndicatorBottom,
                        mIndicatorRight, getHeight()-mIndicatorBottom, mSelectedIndicatorPaint);
            }
        }
    }
