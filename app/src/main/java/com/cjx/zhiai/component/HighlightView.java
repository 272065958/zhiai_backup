...
public class HighlightView {
	...
	final int HYSTERESIS = 20;

	View mContext;
	Matrix mMatrix;
	RectF mImageRect; // in image space
	...
	public HighlightView(CropImageView civ) {
		mContext = civ;
	}
	...
	private void init() {
		...
		mResizeDrawableWidth = ContextCompat.getDrawable(context, R.drawable.camera_crop_width);
		mResizeDrawableHeight = ContextCompat.getDrawable(context, R.drawable.camera_crop_height);
		mResizeDrawableDiagonal = ContextCompat.getDrawable(context, R.drawable.indicator_autocrop);
	}
	...
	public int getEdge(float x, float y) {
		...
		if (mCircle) {
			...
			if (Math.abs(distanceFromCenter - radius) < HYSTERESIS) {
			...
			boolean horizontialCheck = (r.left - HYSTERESIS < x)
					&& (r.right + HYSTERESIS > x);
			boolean verticalCheck = (r.top - HYSTERESIS < y)
					&& (r.bottom + HYSTERESIS > y);
			...
			if ((Math.abs(r.top - y) < HYSTERESIS) && horizontialCheck) {
				retval |= GROW_TOP_EDGE;
			}
			if ((Math.abs(r.bottom - y) < HYSTERESIS) && horizontialCheck) {
				retval |= GROW_BOTTOM_EDGE;
			}
	...
	public void setMode(ModifyMode mm) {
		mMode = mm;
	}
	...
	protected void draw(Canvas canvas) {
		if (hasFocus()) {
			...
			if (mCircle) {
				...
				mOutlinePaint.setColor(0xFF00AEEF);
			} else {
